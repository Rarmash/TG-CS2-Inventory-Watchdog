...
public class Watchdog {
    ...
    public List<Item> scanProfile(String steamid64) throws Exception {
        ...
        if (responseCode != 200) {
            throw new IOException("Failed to fetch inventory data. HTTP error code: " + responseCode);
        }
    ...
    private static double getItemPrice(JSONObject itemJSON) throws IOException {
    ...
    public static void saveToExcel(List<Item> items) throws IOException {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            row.createwCell(2).setCellValue(item.getPrice());
        }
